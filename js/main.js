...
let mouseX, mouseY, scene = null, cubeModel = null, isMouseDown = false, isDragging = false, cubeFaceMat = false, rotation = 0, step = (Math.PI/4.00), envVolume = 0.03, gl = null, gl2 = null, whiteLightMat = null, blueBorderClone = null;
...
const createScene = function () {
    ...
    //load Modelf
    BABYLON.SceneLoader.Append("./assets/", "card_05.glb", scene, function (meshes) {
        ...
        scene.meshes[0].scaling = new BABYLON.Vector3(-4, 4, 4);
        ...
        scene.meshes.forEach(element => {
            if(element.name == 'blue border ') {
                meshArrayGlow.push(element)
                blueBorderClone = element.clone()
                blueBorderClone.name = 'blue border emit'
                meshArrayGlow.push(blueBorderClone)
            }else {
                meshArrayNotGlow.push(element)
            }

            if(element.name == 'Front.001_primitive0')
                element.isVisible = false
            if(element.name == 'Back.001_primitive0')
                element.isVisible = false
            if(element.name == 'ANOTHER WILLIAMS 1-2-1')
                element.isVisible = false
            
            // if(element.name == 'ANOTHER WILLIAMS 1-2' || element.name == 'ANOTHER WILLIAMS 1-2-2')
            //     element.position.z = -0.00
        ...
        scene.materials.forEach(element => {
            ...
            if(element.name == 'Material.016') {
                element.alpha = 0.15
                element.emissiveColor = new BABYLON.Color3.FromHexString("#00453a");
            }

            if(element.name == 'Material.015') {
                element.alpha = .4
            }
            if(element.name == 'Material.003') {
                whiteLightMat = element.clone()
                blueBorderClone.material = whiteLightMat
                whiteLightMat.name = 'whiteLightMat'
                whiteLightMat.id = 'whiteLightMat'
                element.emissiveIntensity = 3;

                whiteLightMat._lightmapTexture = new BABYLON.Texture("./assets/blue-border-emissive-texture.png", scene);
                whiteLightMat.emissiveColor = new BABYLON.Color3.FromHexString("#ffffff");
                element.emissiveIntensity = 3;
                whiteLightMat.hasAlpha = true
                whiteLightMat.alpha = .25
                whiteLightMat.MATERIAL_ALPHABLEND = true
                whiteLightMat.transparencyMode = element.MATERIAL_ALPHABLEND
                element.metallic = 1
                element.roughness = 0
                // element._lightmapTexture.level = 0.2
            }        
            
            if(element.name == 'ANOTHER WILLIAMS 1-2') {
                element.emissiveColor = new BABYLON.Color3.FromHexString("#ededed");
                // element.metallic = 1
                // element.roughness = 0
            }
        });
        ...
        //     if(element.name === "blue border "){
        //         result.set(0, 0.63, 0.52, 1);
        //     }else if (element.name === "blue border emit") {
        //         result.set(0, 0.63, 0.52, 1);
        //     }
        //     else {
        //         result.set(0, 0, 0, 0);
        ...
        let frontImg = new BABYLON.Texture("./assets/front-img.jpg", scene);
        rightImg = new BABYLON.VideoTexture("video", "./assets/right-video.mp4", scene, true);
        rightImg.video.muted = true;
        let leftImg = new BABYLON.Texture("./assets/left-img.jpeg", scene);
        let backImg = new BABYLON.Texture("./assets/back-img.jpeg", scene);
        ...
        if(document.body.classList.contains('isMobile')) {
            mirror.material.reflectionTexture.mirrorPlane = new BABYLON.Plane(0, -1.0, 0, -.56);
        }else {
            mirror.material.reflectionTexture.mirrorPlane = new BABYLON.Plane(0, -1.0, 0, -.5);
        }
        ...
        if(sizes.width / sizes.height >= 1) {
            ...
            scene.meshes[0].scaling = new BABYLON.Vector3(-4,4,4);
            mirrorOverlayPlane.position = new BABYLON.Vector3(0, -1.4, 0);
            mirror.material.reflectionTexture.mirrorPlane.d = -.5
        }else if(sizes.width / sizes.height <= 1) {

            scene.meshes[0].scaling = new BABYLON.Vector3(-2.65,2.65,2.65);
            mirrorOverlayPlane.position = new BABYLON.Vector3(0, -1.5, 0);
            mirror.material.reflectionTexture.mirrorPlane.d = -.35
        }
...
scene.onPointerDown = function(event){
    ...
    animateCube = false
    console.log(cubeModel.rotation.x);
}
...
scene.onPointerMove = function(event){
    ...
    if(isDragging){
        ...
        modelRotationX = cubeModel.rotation.y + ((deltaX * 0.06));
        modelRotationY = cubeModel.rotation.x + ((deltaY * 0.01 * -1));
        if(modelRotationY > 0.175)
            modelRotationY = 0.175
            ...
            if(pickResult.hit) {
                ...
                if(selectedMesh != null) {
                    if (clickedMeshName === selectedMesh.clickable.name) {
                        ...
                        //     }else if(element.name === "blue border "){
                        //         result.set(0, 0.63, 0.52, 1);
                        //     }
                        //     else {
                        //         result.set(0, 0, 0, 0);
